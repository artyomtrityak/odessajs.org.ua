...
function renderSpeakersCarousel() {
  ...
  function loadSpeakerModal($speakerInfoBlock) {
    var $modalBody = $('#speaker-modal'),
      ...
      $modalSpeakerTitle = $modalBody.find('.modal-body__title.report-title'),
      $modalSpeakerText = $modalBody.find('.modal-body__text.report-text'),
      // report
    ...
    if (speakerData) {
      ...
      reports.forEach(function (item, i, arr) {
        reportsContent += '<h2 class="modal-body__title">'+ item.title +'</h2>' + '<p>'+ item.description +'</p>';
      });
